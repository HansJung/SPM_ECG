...
with open('performance_records/performance.csv','w') as csvfile:
    if true_NN and true_SVM:
        fieldnames = ['TN_NN','FP_NN','FN_NN','TP_NN',' ','TN_SVM','FP_SVM','FN_SVM','TP_SVM',' ','TN','FP','FN','TP']
    elif true_SVM:
    ...
    for data_ECG in data:
        for record_idx in data_ECG:
            try:
                obj_main = Main(record_idx=record_idx,alpha=alpha,SDA_L1_penalty=SDA_L1_penalty, SDA_L2_penalty=SDA_L2_penalty, true_SVM=true_SVM, true_NN=true_NN)
                if true_NN and true_SVM:
                    writer.writerow({'TN_NN' : obj_main.NN_accracy_dict['Normal as Normal'], 'FP_NN': obj_main.NN_accracy_dict['Normal as PVC'], 'FN_NN':obj_main.NN_accracy_dict['PVC as Normal'], 'TP_NN':obj_main.NN_accracy_dict['PVC as PVC'], 'TN_SVM' : obj_main.SVM_accracy_dict['Normal as Normal'], 'FP_SVM': obj_main.SVM_accracy_dict['Normal as PVC'], 'FN_SVM':obj_main.SVM_accracy_dict['PVC as Normal'], 'TP_SVM':obj_main.SVM_accracy_dict['PVC as PVC'], 'TN' : obj_main.DictInt_Accuracy['Normal as Normal'], 'FP': obj_main.DictInt_Accuracy['Normal as PVC'], 'FN':obj_main.DictInt_Accuracy['PVC as Normal'], 'TP':obj_main.DictInt_Accuracy['PVC as PVC'] })
                elif true_SVM:
